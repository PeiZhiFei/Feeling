...
/**
 * Encapsulate d behavior with blur effect for app using {@link DialogFragment}.
 * <p>
 * All the screen behind the d will be blurred except the action bar.
 */
...
public abstract class BlurDialogFragment extends DialogFragment {
    ...
    public void onStart() {
        ...
        if (dialog != null) {
            ...
            // add default fade to the d if no window animation has been set.
            int currentAnimation = dialog.getWindow().getAttributes().windowAnimations;
            if (currentAnimation == 0 || S.readBoolear(getActivity(), "3", false)) {
                dialog.getWindow().getAttributes().windowAnimations
                        = R.style.BlurDialogFragment_Default_Animation;
            } else {
                dialog.getWindow().getAttributes().windowAnimations = R.style.dialog_anim2;
            }
        }
        super.onStart();
    }
